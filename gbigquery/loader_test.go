...
func TestLoader_StreamLoad(t *testing.T) {
	...
	spec, err := entity.NewSpec(genericSourceToBigQuerySinkSpec)
...
func TestDynamicColumnCreation(t *testing.T) {
	...
	spec, err := entity.NewSpec(streamSpecWithDynamicColumns)
	assert.NoError(t, err)
	assert.NotNil(t, spec)
...
func TestColumnCloning(t *testing.T) {
	...
	spec, err := entity.NewSpec(streamSpecWithColumnCloneFeature)
	assert.NoError(t, err)
	assert.NotNil(t, spec)
